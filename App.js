...
const CasesPage = ({ user, cases, setCases, evidence: sharedEvidence, setEvidence: setSharedEvidence, warrants: sharedWarrants, setWarrants: setSharedWarrants, onAdd, onAddEvidence, onAddWarrant, setAuditLog }) => {
  const canView = hasPerm(user?.role, "CASE_VIEW");
...
const EvidencePage = ({ user, evidence: evidenceProp, setEvidence: setEvidenceProp, onAdd, setAuditLog }) => {
  const canView = hasPerm(user?.role, "EVIDENCE_VIEW");
  ...
  if (!canView) return (<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:320,gap:16}}>
...
const PleaDealsPage = ({ user }) => {
  const canView = hasPerm(user?.role, "PLEA_VIEW");
...
const BarPage = ({ user, auditLog, audit }) => {
  const canView = hasPerm(user.role, "BAR_VIEW");
...
const DocumentsPage = ({ user, allUsers, cases, auditLog, setAuditLog, audit }) => {
  const canView = hasPerm(user?.role, "DOC_VIEW");
...
const AuthScreen = ({ onLogin, setAuditLog }) => {
...
export default function App() {
  ...
  if (!user) return (
    <>
      ...
      <ToastHost />
      <PdfModal />
      <AuthScreen onLogin={setUser} setAuditLog={setAuditLog} />
    </>
  );
